...
const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:8000/api'
const USAR_BACKEND = import.meta.env.VITE_USAR_BACKEND === 'true'  // ✅ NOVA VAR
...
export async function getUserPapeis(usuarioId) {
  // ✅ Se VITE_USAR_BACKEND for true, tenta backend
  if (USAR_BACKEND) {
    try {
      const response = await fetch(`${API_URL}/usuarios/${usuarioId}/papeis`)
  ...
  console.log('✅ Papéis do usuário (Supabase):', data)
